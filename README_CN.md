...
cd openclaw-trader
npm run setup         # ← 交互式向导：配置凭证、paper 参数、同步 cron

npm run live          # 启动 testnet/实盘监控
npm run paper:status  # 查看账户状态
npm run doctor        # 诊断环境问题
npm test              # 运行测试套件
```

## Testnet 配置（重要）

> ⚠️ Binance 有**两套独立的 Testnet 系统**，API Key 不通用，不能混用。

| 系统 | 地址 | 用途 |
|------|------|------|
| **现货 Testnet** | https://testnet.binance.vision | 现货模拟交易 |
| **合约 Testnet** | https://testnet.binancefuture.com | 合约模拟交易 |

**配置步骤：**

1. **现货 Testnet** — 访问 [testnet.binance.vision](https://testnet.binance.vision)，用 GitHub 账号登录，点"Generate HMAC_SHA256 Key"。保存到 `.secrets/binance-testnet.json`。

2. **合约 Testnet** — 在 [testnet.binancefuture.com](https://testnet.binancefuture.com) 单独注册账号。API Keys → 创建。保存到 `.secrets/binance-futures-testnet.json`。
...
**已知 Testnet 限制**（系统已自动处理，无需手动干预）：
- 现货 Testnet 最多允许 5 个同时存在的止损/止盈挂单（`MAX_NUM_ALGO_ORDERS`）。超出部分自动降级为本地价格轮询止损。
- 合约 Testnet 不支持通过标准 REST API 挂条件单。系统自动降级为本地价格轮询止损。
...
### Kill Switch 意外触发
Kill switch 在总亏损超过配置阈值时激活（合约默认 20%，现货 testnet 默认 80%）。
...
**常见原因与修复：**
- **State 文件 `initialUsdt` 与 paper.yaml 不一致**：P&L 计算基准错误，导致误判。修复：
  ```bash
  npm run paper:reset -- --scenario testnet-default --set-initial 3000
  ```
- **持仓对账清零**：重启时若交易所找不到本地记录的持仓（如被手动平仓），本地状态被清零，权益骤降触发 kill switch。修复：检查交易所实际持仓，手动同步 state 文件或执行 `paper:reset`。

### `initialUsdt` 显示异常值
`logs/paper-*.json` 中的 `initialUsdt` 独立于 `paper.yaml` 存储。如果两者不一致：
```bash
npm run paper:reset -- --scenario <id> --set-initial <正确金额>
```

### 合约 Testnet 报 `-2015` 错误
你在合约端点使用了现货 Testnet 的 Key。两套系统 Key 不通用，参见上方"Testnet 配置"章节。

### Cron 任务未运行
```bash
npm run cron:sync    # 重新注册所有定时任务
npm run cron:list    # 验证已注册的任务
```
