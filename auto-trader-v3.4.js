...
async function syncStateToDashboard(balance, positions) {
  try {
    ...
    console.log("⚠️ State sync error: " + e.message);
  }
}
...
function apiRequest(host, apiPath, apiKey, secretKey, params = {}) {
  return new Promise((resolve, reject) => {
    ...
    const options = {
      hostname: host,
      path: apiPath + "?" + query + "&signature=" + sig,
      method: "GET",
...
async function getEntrySignal(symbol) {
  ...
  return { price: latestClose, rsi, bb, atriaStop, atriaTP, pattern, volatility };
}
...
async function analyzeSymbol(symbol, balance, positions) {
  ...
  const { price, rsi, bb, atriaStop, atriaTP, pattern, volatility } = entry;
  ...
  if (posSide === "LONG" && pos) {
    ...
    if (holdMinutes >= MAX_HOLD_MINUTES) {
      return { action: "CLOSE_LONG", qty: posAmt, reason: "超时" + holdMinutes.toFixed(0) + "分钟", price };
    }
    
    // 止盈止损
    const stopLossPct = STOP_LOSS_PCT * 100;
    const takeProfitPct = TAKE_PROFIT_PCT * 100;
    if (pnlPct >= takeProfitPct) {
      return { action: "CLOSE_LONG", qty: posAmt, reason: "止盈 " + pnlPct.toFixed(2) + "%", price };
    }
    if (pnlPct <= -stopLossPct) {
      return { action: "CLOSE_LONG", qty: posAmt, reason: "止损 " + pnlPct.toFixed(2) + "%", price };
    }
  }
  ...
  if (posSide === "SHORT" && pos) {
    ...
    if (holdMinutes >= MAX_HOLD_MINUTES) {
      return { action: "CLOSE_SHORT", qty: Math.abs(posAmt), reason: "超时" + holdMinutes.toFixed(0) + "分钟", price };
    }
    
    const stopLossPct = STOP_LOSS_PCT * 100;
    const takeProfitPct = TAKE_PROFIT_PCT * 100;
    if (pnlPct >= takeProfitPct) {
      return { action: "CLOSE_SHORT", qty: Math.abs(posAmt), reason: "止盈 " + pnlPct.toFixed(2) + "%", price };
    }
    if (pnlPct <= -stopLossPct) {
      return { action: "CLOSE_SHORT", qty: Math.abs(posAmt), reason: "止损 " + pnlPct.toFixed(2) + "%", price };
    }
  }
  ...
  if (trendDir === "UP" && positions.length < MAX_POSITIONS) {
    ...
    if (buySignal) {
      if (volatility < minVol) {
        return { action: "HOLD", qty: 0, reason: "波动率" + volatility.toFixed(2) + "%<" + (minVol * 100).toFixed(1) + "%", price };
      }
      let signalDesc = "RSI=" + rsi.toFixed(1) + " 触布林下轨";
      if (pattern && pattern.bullish) signalDesc += " " + pattern.pattern;
      return { action: "BUY_LONG", qty: 0, stopLoss: price - atriaStop, takeProfit: price + atriaTP, reason: signalDesc, price };
    }
  }
  ...
  if (trendDir === "DOWN" && positions.length < MAX_POSITIONS) {
    ...
    if (sellSignal) {
      ...
      if (pattern && !pattern.bullish) signalDesc += " " + pattern.pattern;
      return { action: "SELL_SHORT", qty: 0, stopLoss: price + atriaStop, takeProfit: price - atriaTP, reason: signalDesc, price };
    }
  }
      ...
      try {
        ...
        if (action === "HOLD") {
          ...
          if (action === "CLOSE_LONG") {
            ...
            if (tradeQty < step) {
              console.log("  ⚠️ " + sym + ": 数量" + tradeQty + "低于最小" + step);
              continue;
            }
            
            if (action === "BUY_LONG") {
              await placeOrder(sym, "BUY", "LONG", tradeQty);
              console.log("  🟢 " + sym + ": 开多 @$" + price.toFixed(4) + " | SL:$" + analysis.stopLoss.toFixed(4) + " TP:$" + analysis.takeProfit.toFixed(4));
            } else {
              await placeOrder(sym, "SELL", "SHORT", tradeQty);
              console.log("  🔴 " + sym + ": 开空 @$" + price.toFixed(4) + " | SL:$" + analysis.stopLoss.toFixed(4) + " TP:$" + analysis.takeProfit.toFixed(4));
            }
          }
        }
      } catch (e) {
