 ...
 */
export function calculateMetrics(trades, initialUsdt, equityCurve, btcBenchmarkReturn) {
    ...
    const sharpeRatio = stdPeriodReturn > 0 ? (avgPeriodReturn / stdPeriodReturn) * Math.sqrt(periodReturns.length) : 0;
    ...
    const downDev = downReturns.length > 0
        ...
        : 0;
    const sortinoRatio = downDev > 0 ? (avgPeriodReturn / downDev) * Math.sqrt(periodReturns.length) : 0;
    // ── Average statistics ──
    const avgWinPct = wins.length > 0 ? (wins.reduce((s, t) => s + t.pnlPercent, 0) / wins.length) * 100 : 0;
    ...
    // ── Benchmark & Alpha ──
    const alpha = btcBenchmarkReturn !== undefined ? totalReturnPct - btcBenchmarkReturn : undefined;
    return {
        ...
        avgHoldingHours: sellTrades.length > 0
        ...
        bestTradePct: sellTrades.length > 0 ? Math.max(...sellTrades.map((t) => t.pnlPercent * 100)) : 0,
        worstTradePct: sellTrades.length > 0 ? Math.min(...sellTrades.map((t) => t.pnlPercent * 100)) : 0,
        calmarRatio,
