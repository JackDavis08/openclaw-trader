...
function calcEquity(account, prices) {
...
function doBuy(account, symbol, price, time, cfg, opts, signalConditions = []) {
    // Skip if already holding position
    if (account.positions[symbol])
        return;
    // Max positions check
    if (Object.keys(account.positions).length >= cfg.risk.max_positions)
        return;
    // Daily loss limit
    ...
    if (symbolValue / equity >= cfg.risk.max_position_per_symbol)
        return;
    if ((account.dailyLoss.loss / equity) * 100 >= cfg.risk.daily_loss_limit_percent)
        return;
    const usdtToSpend = equity * cfg.risk.position_ratio;
    if (usdtToSpend < cfg.execution.min_order_usdt)
        return;
...
function doOpenShort(account, symbol, price, time, cfg, opts, signalConditions = []) {
    if (account.positions[symbol])
        return;
    if (Object.keys(account.positions).length >= cfg.risk.max_positions)
        return;
    ...
    if (marginToLock < cfg.execution.min_order_usdt || marginToLock > account.usdt)
        return;
...
function doCoverShort(account, symbol, exitPrice, exitTime, exitReason, opts) {
...
function doSell(account, symbol, exitPrice, exitTime, exitReason, opts) {
    const pos = account.positions[symbol];
    if (!pos)
        return;
...
function updateTrailingStop(pos, high, low, cfg) {
    if (!pos.trailingStop || !cfg.risk.trailing_stop.enabled)
        return false;
    ...
    if (pos.side === "short") {
        ...
        if (high > ts.highestPrice)
            ts.highestPrice = high;
 ...
 */
function checkIntracandleExit(pos, kline, cfg, time, trailingTriggered, useIntracandle) {
    ...
    const roiThreshold = roiTable !== undefined && Object.keys(roiTable).length > 0
...
function getFundingSettlements(lastTs, currentTs) {
    const result = [];
...
function findFundingRate(history, settlementTs) {
    // Find the most recent record where ts <= settlementTs
    let best;
    for (const r of history) {
        if (r.ts <= settlementTs)
            best = r;
        else
            break;
    }
...
export function runBacktest(klinesBySymbol, cfg, opts = {}, trendKlinesBySymbol) {
    const { initialUsdt = 1000, feeRate = 0.001, slippagePercent = 0.05, avgFundingRatePer8h, fundingHistory, intracandle = true, signalToNextOpen = false } = opts;
    ...
    const timeSets = symbols.map((s) => new Set((klinesBySymbol[s] ?? []).map((k) => k.openTime)));
    const allTimes = Array.from(timeSets[0] ?? new Set())
    ...
    const trendSorted = {};
    const trendWindowPos = {}; // index of processed candles
    const trendWindows = {};
    if (useMtf) {
        for (const sym of symbols) {
            trendSorted[sym] = [...(trendKlinesBySymbol[sym] ?? [])].sort((a, b) => a.closeTime - b.closeTime);
            trendWindowPos[sym] = 0;
            trendWindows[sym] = [];
        }
    }
    /** Get trend MA status for sym at time (bullish=true, bearish=false, no data=null) */
    function getTrendBull(sym, time) {
        if (!useMtf)
            return null;
        ...
        while (nextPos < sorted.length && (sorted[nextPos]?.closeTime ?? Infinity) < time) {
            ...
            if (trendKline)
                trendWindows[sym]?.push(trendKline);
            nextPos++;
        }
        trendWindowPos[sym] = nextPos;
        const tw = trendWindows[sym] ?? [];
        if (tw.length < trendWarmup)
            return null; // insufficient data, allow through
        const ind = calculateIndicators(tw, cfg.strategy.ma.short, cfg.strategy.ma.long, cfg.strategy.rsi.period, { enabled: false, fast: 12, slow: 26, signal: 9 } // MTF only needs MA, no MACD
        );
        if (!ind)
            return null;
    ...
    for (const time of allTimes) {
        ...
        if (Object.values(windows).every((w) => w.length < warmupBars))
            continue;
        ...
        for (const sym of symbols) {
            ...
            if (needsCtx) {
                ...
                const ind = calculateIndicators(win, cfg.strategy.ma.short, cfg.strategy.ma.long, cfg.strategy.rsi.period, cfg.strategy.macd);
                if (ind) {
                    stratCtx = { klines: win, cfg, indicators: ind, currentPosSide: posSide };
                }
            }
            // ── Helper: execute exit and call onTradeClosed ─────────────────
            const executeExit = (exitPrice, exitReason) => {
                if (posSide === "short") {
            ...
            if (strategy?.shouldExit !== undefined && stratCtx !== undefined) {
                const shouldExitResult = strategy.shouldExit({ symbol: sym, side: posSide, entryPrice: pos.entryPrice, currentPrice, holdMs }, stratCtx);
                if (shouldExitResult?.exit) {
                    // confirmExit check
                    const maxDev = cfg.execution.max_exit_price_deviation ?? 0.15;
                    const confirmResult = shouldConfirmExit({ symbol: sym, side: posSide, entryPrice: pos.entryPrice, currentPrice, profitRatio, holdMs }, shouldExitResult.reason, maxDev, strategy, stratCtx);
            ...
            {
                const newStop = resolveNewStopLoss(posSide, pos.entryPrice, pos.stopLoss, currentPrice, profitRatio, holdMs, sym, cfg.risk, strategy, stratCtx);
                if (newStop !== null) {
            ...
            const exitResult = checkIntracandleExit(pos, kline, cfg, time, trailingTriggered, intracandle);
            if (exitResult) {
                // ── Hook Priority 3: confirmExit ─────────────────────────
                const useConfirmExit = strategy !== undefined || cfg.execution.max_exit_price_deviation !== undefined;
                if (useConfirmExit) {
                    const maxDev = cfg.execution.max_exit_price_deviation ?? 0.15;
                    const confirmResult = shouldConfirmExit({ symbol: sym, side: posSide, entryPrice: pos.entryPrice, currentPrice, profitRatio, holdMs }, exitResult.reason, maxDev, strategy, stratCtx);
                    if (!confirmResult.confirmed) {
        ...
        for (const sym of symbols) {
            ...
            if (cfg.risk.correlation_filter?.enabled) {
                for (const heldSym of Object.keys(account.positions)) {
                    ...
                    if (heldWin)
                        heldKlinesBySymbol[heldSym] = heldWin;
                }
            }
            ...
            if (signal.type === "buy") {
                ...
                const trendBull = getTrendBull(sym, time);
                if (trendBull === true)
                    continue; // macro trend is bullish, don't open short
                if (signalToNextOpen) {
    ...
    for (const sym of Object.keys(account.positions)) {
        ...
        if (!lastKline || !pos)
            continue;
        const posSideForClose = pos.side ?? "long";
        if (pos.side === "short") {
        ...
        if (strategy?.onTradeClosed !== undefined) {
            ...
            const ind = calculateIndicators(win, cfg.strategy.ma.short, cfg.strategy.ma.long, cfg.strategy.rsi.period, cfg.strategy.macd);
            if (ind) {
                const closeCtx = { klines: win, cfg, indicators: ind, currentPosSide: posSideForClose };
                const lastTrade = account.trades[account.trades.length - 1];
                if (lastTrade !== undefined) {
                    const tradeResult = {
                        symbol: sym,
