...
export function loadStrategyConfig() {
    return readYaml(path.join(CONFIG_DIR, "strategy.yaml"));
}
export function loadPaperConfig() {
    return readYaml(path.join(CONFIG_DIR, "paper.yaml"));
}
export function loadLiveConfig() {
    return readYaml(path.join(CONFIG_DIR, "live.yaml"));
}
export function loadStrategyProfile(strategyId) {
...
// ─────────────────────────────────────────────────────
export function mergeRisk(base, ...overrides) {
    let result = { ...base };
    for (const override of overrides) {
        ...
        result = {
            ...
            ...(result.atr_position !== undefined || override.atr_position !== undefined
                ? { atr_position: { ...result.atr_position, ...override.atr_position } }
                : {}),
            ...(result.correlation_filter !== undefined || override.correlation_filter !== undefined
                ? { correlation_filter: { ...result.correlation_filter, ...override.correlation_filter } }
                : {}),
            ...(result.dca !== undefined || override.dca !== undefined
                ? { dca: { ...result.dca, ...override.dca } }
                : {}),
        };
    }
    return result;
}
export function mergeStrategySection(base, override) {
    if (!override)
        return base;
 ...
 */
export function buildPaperRuntime(base, paperCfg, scenario) {
 ...
 */
export function buildLiveRuntime(base, live) {
    if (!live.exchange) {
 ...
 */
export function loadEnabledPaperRuntimes() {
 ...
 */
export function buildLiveAccountRuntimes(account, base, paperCfg) {
 ...
 */
export function loadLiveAccountConfigs() {
    const liveCfg = loadLiveConfig();
    if (!liveCfg.accounts || liveCfg.accounts.length === 0)
        return [];
    const base = loadStrategyConfig();
    const paperCfg = loadPaperConfig();
    return liveCfg.accounts.flatMap((account) => buildLiveAccountRuntimes(account, base, paperCfg));
}
/**
