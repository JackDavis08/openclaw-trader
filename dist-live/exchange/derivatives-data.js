...
// ─── Utility Functions ──────────────────────────────────────────
function fetchJson(url, ipv4 = false) {
    return new Promise((resolve, reject) => {
        ...
        const opts = {
            ...
            ...(ipv4 ? { agent: new https.Agent({ family: 4 }) } : {}),
        };
        const req = https.request(opts, (res) => {
            ...
            res.on("end", () => {
                try {
                    ...
                    reject(new Error(`JSON parse error from ${parsed.hostname}`));
                }
            });
        });
        req.on("error", reject);
...
export async function getLongShortRatio(symbol) {
    ...
    const [global, topAcc, topPos] = await Promise.allSettled([
        fetchJson(`${baseUrl}/globalLongShortAccountRatio?symbol=${symbol}&period=5m&limit=1`),
        fetchJson(`${baseUrl}/topLongShortAccountRatio?symbol=${symbol}&period=5m&limit=1`),
        fetchJson(`${baseUrl}/topLongShortPositionRatio?symbol=${symbol}&period=5m&limit=1`),
    ]);
...
function calcMaxPain(options, spot) {
    ...
    const strikes = [...new Set(expiryOptions.map((o) => parseOptionName(o.instrument_name)?.strike ?? 0).filter((s) => s > 0))].sort((a, b) => a - b);
    if (strikes.length === 0)
        return { price: spot, expiry: bestExpiry };
...
function calcAtmIv(options, spot) {
 ...
 */
function calcPCR(options, nearbyDays = 0) {
    let putOI = 0;
    let callOI = 0;
    const now = Date.now();
    ...
    for (const opt of options) {
        ...
        if (nearbyDays > 0) {
            ...
            if (expiryTs > cutoff || expiryTs < now)
                continue;
        }
        if (parsed.isCall)
 ...
 */
export async function getOptionsData(currency) {
    const data = await fetchJson(`https://www.deribit.com/api/v2/public/get_book_summary_by_currency?currency=${currency}&kind=option`);
...
function calcIvSmile(options, spot, expiryFilter) {
    ...
    for (const otmPct of OTM_LEVELS) {
        ...
        const wAvgIv = (opts) => {
            ...
            if (totalOI === 0)
                return opts.reduce((s, o) => s + o.mark_iv, 0) / opts.length;
...
function calcTermStructure(options, spot) {
    const now = Date.now();
    const expiryMap = new Map();
    for (const opt of options) {
    ...
    for (const [expiry, opts] of expiryMap) {
        ...
        if (dte > 365)
            continue; // Skip options over 1 year out
 ...
 */
export async function getIvSkewData(currency) {
    const data = await fetchJson(`https://www.deribit.com/api/v2/public/get_book_summary_by_currency?currency=${currency}&kind=option`);
    const options = data.result.filter((o) => o.mark_iv > 0 && o.open_interest > 0);
    if (options.length === 0)
        throw new Error(`No IV data for ${currency}`);
    const spot = options[0]?.underlying_price ?? 0;
    const now = Date.now();
    ...
    const nearestExpiry = (() => {
        const futureOpts = options
            ...
            .filter((p) => p !== null && parseExpiryDate(p.expiry) > now)
...
export function formatIvSkewReport(skew) {
    const lines = [`📐 **${skew.currency} IV Skew Analysis**\n`];
    // Smile table
...
export async function getDerivativesSnapshot(symbol, includeSkew = false) {
...
// ─── Format Report ───────────────────────────────────────
export function formatDerivativesReport(snap) {
    const coin = snap.symbol.replace("USDT", "");
    const lines = [`📈 **${coin} Derivatives Market**\n`];
    if (snap.basis) {
        const b = snap.basis;
...
export function formatMultiDerivativesReport(snaps) {
