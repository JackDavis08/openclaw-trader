...
import { loadAccount, saveAccount, resetDailyLossIfNeeded, calcTotalEquity, registerOrder, confirmOrder, getTimedOutOrders, cancelOrder, cleanupOrders, } from "../paper/account.js";
...
function orderToPaperTrade(order, side, reason, pnl, pnlPercent, fallbackPrice // Used when fills=[]/price=0 (Futures Testnet MARKET order quirk)
) {
...
export class LiveExecutor {
    ...
    async handleBuy(signal) {
        ...
        if (atrCfg?.enabled && signal.indicators.atr) {
            usdtToSpend = calcAtrPositionSize(equity, signal.price, signal.indicators.atr, atrCfg.risk_per_trade_percent / 100, atrCfg.atr_multiplier, atrCfg.max_position_ratio);
        }
        else {
        ...
        const exchangeSlOrderId = await this.placeExchangeStopLoss(signal.symbol, "long", execQty, stopLossPrice);
        let takeProfitOrderId;
        try {
            const tpOrder = await this.client.placeTakeProfitOrder(signal.symbol, "SELL", execQty, takeProfitPrice);
            // orderId=-1 means degraded to local polling take profit (LOCAL_ONLY)
            if (tpOrder.orderId !== -1)
        ...
        console.log(`${label} Buy ${signal.symbol}: qty=${execQty.toFixed(6)}, avgPrice=$${avgPrice.toFixed(4)}, fee=$${totalFee.toFixed(4)}, ${slLabel}`);
     ...
     */
    async handleSell(symbol, currentPrice, reason) {
        const account = loadAccount(this.cfg.paper.initial_usdt, this.scenarioId);
        ...
        if (position.takeProfitOrderId !== undefined) {
            try {
                await this.client.cancelOrder(symbol, position.takeProfitOrderId);
            }
        ...
        if (position.exchangeSlOrderId !== undefined) {
            ...
            try {
                await this.client.cancelOrder(symbol, position.stopLossOrderId);
            }
        ...
        const label = this.isTestnet ? "[TESTNET]" : "[LIVE]";
        console.log(`${label} Sell ${symbol}: qty=${execQty.toFixed(6)}, avgPrice=$${avgPrice.toFixed(4)}, ` +
    ...
    async handleShort(signal) {
        ...
        const atrCfg = this.cfg.risk.atr_position;
        if (atrCfg?.enabled && signal.indicators.atr) {
            marginToLock = calcAtrPositionSize(equity, signal.price, signal.indicators.atr, atrCfg.risk_per_trade_percent / 100, atrCfg.atr_multiplier, atrCfg.max_position_ratio);
        }
        else {
        ...
        const isFuturesTestnetZeroFill = this.isTestnet &&
            this.cfg.exchange.market === "futures" &&
            reportedQty === 0;
        let execQty;
        if (isFuturesTestnetZeroFill) {
            console.warn(`${label} ℹ️ Futures testnet quirk: ${signal.symbol} MARKET short returned 0 fills ` +
        ...
        const shortExchangeSlOrderId = await this.placeExchangeStopLoss(signal.symbol, "short", execQty, shortStopLoss);
        let shortTpOrderId;
        try {
     ...
     */
    async handleCover(symbol, currentPrice, reason) {
        const account = loadAccount(this.cfg.paper.initial_usdt, this.scenarioId);
        const position = account.positions[symbol];
     ...
     */
    async checkExitConditions(prices) {
        const account = loadAccount(this.cfg.paper.initial_usdt, this.scenarioId);
        resetDailyLossIfNeeded(account);
        const results = [];
        for (const [symbol, pos] of Object.entries(account.positions)) {
            ...
            let exchangeTriggeredReason = null;
            let exchangeTriggeredPrice = currentPrice;
            for (const [orderId, reason] of [
                [pos.stopLossOrderId, "stop_loss"],
                [pos.takeProfitOrderId, "take_profit"],
            ]) {
                ...
                try {
                    ...
                    if (orderStatus.status === "FILLED") {
                        ...
                        if (fills && fills.length > 0) {
                            exchangeTriggeredPrice = fills.reduce((s, f) => s + parseFloat(f.price) * parseFloat(f.qty), 0) / parseFloat(orderStatus.executedQty);
                        }
            ...
            // ── P8.1 Break-Even Stop / Custom Stoploss ──
            {
                ...
                const newStop = resolveNewStopLoss(pos.side ?? "long", pos.entryPrice, pos.stopLoss, currentPrice, profitRatio, holdMs, symbol, this.cfg.risk, this.strategy, undefined // StrategyContext not available in executor loop
                );
                if (newStop !== null) {
                    ...
                    if (pos.exchangeSlOrderId !== undefined) {
                        ...
                        const newSlOrderId = await this.placeExchangeStopLoss(symbol, pos.side ?? "long", pos.quantity, newStop);
                    ...
                    saveAccount(account, this.scenarioId);
                    const label = this.isTestnet ? "[TESTNET]" : "[LIVE]";
                    console.log(`${label} [break-even] ${symbol} SL updated: $${oldStop.toFixed(4)} → $${newStop.toFixed(4)}`);
                }
            }
            ...
            const hitRoiTable = roiTable !== undefined &&
            ...
            if (exitReason) {
                // ── P8.2 Exit confirmation hook ──────────────────────────────────
                {
                    ...
                    const confirmResult = shouldConfirmExit({ symbol, side: pos.side ?? "long", entryPrice: pos.entryPrice, currentPrice, profitRatio, holdMs }, exitReason, maxDev, this.strategy, undefined // StrategyContext not available in executor loop
                    );
                    if (!confirmResult.confirmed) {
                        const cooling = isExitRejectionCoolingDown(symbol, cooldownSec * 1000, this._exitRejectionLog);
                        if (!cooling) {
                            console.log(`[confirm-exit] ${symbol} exit rejected (reason: ${confirmResult.reason ?? "unknown"}, exitReason: ${exitReason})`);
     ...
     */
    async checkOrderTimeouts(account) {
        const label = this.isTestnet ? "[TESTNET]" : "[LIVE]";
        const timedOut = getTimedOutOrders(account);
        if (timedOut.length === 0)
            return;
        ...
        for (const pending of timedOut) {
            try {
                ...
                if (status === "FILLED") {
                    ...
                    confirmOrder(account, pending.orderId, parseFloat(orderStatus.executedQty), pending.requestedQty);
                    console.log(`${label} Timed-out order #${pending.orderId} (${pending.symbol}) filled, local state synced qty=${orderStatus.executedQty}`);
                }
                else if (status === "PARTIALLY_FILLED") {
                    // Partially filled -> record actual executed quantity
                    confirmOrder(account, pending.orderId, parseFloat(orderStatus.executedQty), pending.requestedQty);
                    console.log(`${label} Timed-out order #${pending.orderId} (${pending.symbol}) partially filled ${orderStatus.executedQty}/${pending.requestedQty.toFixed(6)}`);
                }
                else if (status === "NEW") {
                    ...
                    await this.client.cancelOrder(pending.symbol, pending.orderId);
                    cancelOrder(account, pending.orderId);
                    console.log(`${label} Timed-out ${typeLabel} order #${pending.orderId} (${pending.symbol}) cancelled. ` +
                    ...
                    if (!isEntry) {
                        ...
                        if (pos) {
                            ...
                            if (pos.exitTimeoutCount >= EXIT_TIMEOUT_MAX_RETRIES) {
                                console.warn(`[ForceExit] ${pending.symbol} exit timed out ${EXIT_TIMEOUT_MAX_RETRIES} times, triggering forced market liquidation`);
                    ...
                    cancelOrder(account, pending.orderId);
                    console.log(`${label} Order #${pending.orderId} (${pending.symbol}) status=${status}, cleaning up local record`);
                }
            }
            catch (err) {
                console.warn(`${label} checkOrderTimeouts: failed to process order #${pending.orderId} (${pending.symbol}):`, err instanceof Error ? err.message : err);
            }
        }
        cleanupOrders(account);
        saveAccount(account, this.scenarioId);
     ...
     */
    async placeExchangeStopLoss(symbol, side, qty, stopPrice) {
        try {
            ...
            if (slOrder.orderId === -1)
                return null;
            return slOrder.orderId;
        }
        catch (err) {
            console.warn(`[LiveExecutor] Native stop loss order placement failed ${symbol} (${side}):`, err instanceof Error ? err.message : err);
            return null;
        }
    }
    /**
     ...
     */
    async selfHealClosedPosition(symbol, position, account, currentPrice, reason) {
        const label = this.isTestnet ? "[TESTNET]" : "[LIVE]";
        try {
            ...
            if (!slStillOpen && slOrderId !== undefined) {
                ...
                account.trades.push(trade);
                saveAccount(account, this.scenarioId);
                console.log(`${label} [self-heal] ${symbol} position confirmed closed on exchange. ` +
     ...
     */
    async cancelExchangeStopLoss(symbol, orderId) {
        try {
            await this.client.cancelOrder(symbol, orderId);
        }
        catch (err) {
            console.warn(`[LiveExecutor] Cancel native stop loss order failed ${symbol} #${orderId}:`, err instanceof Error ? err.message : err);
        }
    }
    /**
     ...
     */
    async syncExchangeStopLosses(account, scenarioId) {
        const label = this.isTestnet ? "[TESTNET]" : "[LIVE]";
        for (const [symbol, pos] of Object.entries(account.positions)) {
            if (pos.exchangeSlOrderId === undefined)
                continue;
            try {
                ...
                if (status === "FILLED") {
                    ...
                    if (fills && fills.length > 0) {
                        exitPrice = fills.reduce((s, f) => s + parseFloat(f.price) * parseFloat(f.qty), 0) / parseFloat(orderStatus.executedQty);
                    }
                    console.log(`${label} [syncExchangeStopLosses] ${symbol} native stop loss order #${pos.exchangeSlOrderId} triggered @ $${exitPrice.toFixed(4)}`);
                    const isShort = pos.side === "short";
                    const pnl = isShort
                    ...
                    console.warn(`${label} [syncExchangeStopLosses] ${symbol} native stop loss order #${pos.exchangeSlOrderId} abnormal status: ${status} (local polling as fallback)`);
                }
                // NEW / PARTIALLY_FILLED -> no action
            }
            catch (err) {
                console.warn(`${label} [syncExchangeStopLosses] query ${symbol} stop loss order #${pos.exchangeSlOrderId} failed:`, err instanceof Error ? err.message : err);
            }
        }
     ...
     */
    async checkDcaTranches(prices, ctx) {
        ...
        for (const [symbol, pos] of Object.entries(account.positions)) {
            ...
            if (this.strategy?.adjustPosition !== undefined && ctx !== undefined) {
                const adjustAmount = this.strategy.adjustPosition({ symbol, side, entryPrice: pos.entryPrice, currentPrice, quantity: pos.quantity, costBasis, profitRatio, holdMs, dcaCount }, ctx);
     ...
     */
    async forceExit(account, position, scenarioId, reason) {
        ...
        try {
            ...
            if (fills && fills.length > 0) {
                exitPrice = fills.reduce((s, f) => s + parseFloat(f.price) * parseFloat(f.qty), 0) / parseFloat(exitOrder.executedQty);
            }
            ...
            account.trades.push(trade);
            console.log(`${label} [ForceExit] ${symbol} forced exit: price=$${exitPrice.toFixed(4)}, PnL=${pnl >= 0 ? "+" : ""}$${pnl.toFixed(2)}, reason=${reason}`);
        }
        catch (err) {
            console.error(`${label} [ForceExit] ${symbol} force exit order failed:`, err instanceof Error ? err.message : err);
        }
        ...
        try {
            const reasonLabel = reason === "force_exit_timeout"
                ...
                : "Manual force exit";
            sendTelegramMessage(`⚠️ [ForceExit] ${symbol} Forced market exit\nReason: ${reasonLabel}\nExit price: $${exitPrice.toFixed(4)}`);
        }
 ...
 */
export function createLiveExecutor(cfg, exchange) {
