...
// ─── File IO ──────────────────────────────────────────
export function readEmergencyHalt() {
    try {
        const state = JSON.parse(fs.readFileSync(EMERGENCY_PATH, "utf-8"));
...
export async function checkEmergencyNews() {
    ...
    const seenUrls = existing.seenUrls ?? {};
