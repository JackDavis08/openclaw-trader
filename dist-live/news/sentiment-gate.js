...
import { evaluateCachedSentiment } from "./sentiment-cache.js";
 ...
 */
export function scoreNewsTitles(titles) {
    let score = 0;
...
export function loadNewsReport() {
    try {
 ...
 */
export function evaluateSentimentGate(signal, report, baseRatio, sentimentCache) {
    ...
    const newsScoreLabel = newsScore > 0 ? `+${newsScore} bullish` : newsScore < 0 ? `${newsScore} bearish` : "0 neutral";
