...
function computeDriftBounds(baseline, space, maxDriftPercent) {
    const bounds = {};
...
export class AdaptiveManager {
    ...
    arms = [];
    activeArmId = "arm-0";
    tradesSinceRefresh = 0;
    totalTradeCount = 0;
    rng;
    constructor(scenarioId, baselineParams, config, paramSpace) {
    ...
    initialize() {
        ...
        if (fs.existsSync(statePath)) {
            try {
                ...
                const baselineMatch = Object.keys(this.baselineParams).every((k) => Math.abs((state.baselineParams[k] ?? 0) - (this.baselineParams[k] ?? 0)) < 1e-9);
    ...
    createFreshArms() {
        ...
        for (let i = 1; i < this.config.num_arms; i++) {
            const params = perturbWithinBounds(this.baselineParams, this.paramSpace, bounds, 0.15, // Initial perturbation sigma
            this.rng);
    ...
    getActiveParams() {
        ...
        const hasQualified = this.arms.some((a) => a.totalTrades >= this.config.min_trades_per_arm);
        if (!hasQualified) {
            this.activeArmId = "arm-0";
        ...
        if (this.config.fallback_on_underperform) {
            const qualified = this.arms.filter((a) => a.totalTrades >= this.config.min_trades_per_arm);
    ...
    onTradeClosed(trade) {
        ...
        if (armIdx < 0)
            return; // Unknown arm (possibly from before a reset)
    ...
    refreshArms() {
        if (this.arms.length <= 2)
            return; // Need at least 3 arms to do meaningful refresh
        ...
        this.arms = this.arms.map((arm) => {
            if (!retireIds.has(arm.id))
                return arm;
            const newParams = perturbWithinBounds(bestArm.params, this.paramSpace, bounds, 0.1, this.rng);
