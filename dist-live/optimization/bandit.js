...
function sampleGamma(shape, rng) {
    ...
    for (let iter = 0; iter < 1000; iter++) {
        ...
        if (u < 1 - 0.0331 * x * x * x * x)
            return d * v;
        if (Math.log(Math.max(u, 1e-15)) < 0.5 * x * x + d * (1 - v + Math.log(v)))
            return d * v;
    }
 ...
 */
export function selectArm(arms, config, rng) {
    if (arms.length === 0) {
 ...
 */
export function updateArm(arm, pnlPercent) {
 ...
 */
export function createArm(id, params, priorAlpha = 1, priorBeta = 1) {
    return {
        id,
        params,
        alpha: priorAlpha,
