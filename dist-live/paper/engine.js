...
const _exitRejectionLog = new Map();
function getDb(scenarioId) {
...
import { loadAccount, saveAccount, paperBuy, paperDcaAdd, paperSell, paperOpenShort, paperCoverShort, calcTotalEquity, getAccountSummary, updateTrailingStop, resetDailyLossIfNeeded, } from "./account.js";
const log = createLogger("engine");
function scenarioId(cfg) {
    return cfg.paper.scenarioId;
}
function paperOpts(cfg) {
    return {
 ...
 */
export function handleSignal(signal, cfg) {
    ...
    if (signal.type === "buy") {
        ...
        if (!skipped) {
            ...
            let overridePositionUsdt;
            const atrCfg = cfg.risk.atr_position;
            if (atrCfg?.enabled && signal.indicators.atr) {
                const equity = calcTotalEquity(account, { [signal.symbol]: signal.price });
                overridePositionUsdt = calcAtrPositionSize(equity, signal.price, signal.indicators.atr, atrCfg.risk_per_trade_percent / 100, atrCfg.atr_multiplier, atrCfg.max_position_ratio);
            }
            trade = paperBuy(account, signal.symbol, signal.price, signal.reason.join(", "), {
                ...paperOpts(cfg),
            ...
            if (newPos) {
                ...
                try {
                    ...
                    log.warn(`Signal history log failed: ${e instanceof Error ? e.message : String(e)}`);
                }
                // ── G5: SQLite persistence (optional) ──
                if (cfg.paper.use_sqlite === true && trade) {
                    try {
                        const db = getDb(cfg.paper.scenarioId);
                        newPos.dbId = db.insertTrade(cfg.paper.scenarioId, signal.symbol, "buy", newPos.quantity, trade.price, newPos.stopLoss, newPos.takeProfit, trade.timestamp);
                    }
                    catch (e) {
                        log.warn(`SQLite insert failed: ${e instanceof Error ? e.message : String(e)}`);
                    }
                }
        ...
        if (trade && sigHistId) {
            try {
                closeSignal(sigHistId, signal.price, "signal", trade.pnl);
            }
            catch (e) {
                log.warn(`Signal history close failed: ${e instanceof Error ? e.message : String(e)}`);
            }
        }
    }
        ...
        if (!skipped) {
            ...
            trade = paperOpenShort(account, signal.symbol, signal.price, signal.reason.join(", "), {
                ...paperOpts(cfg),
                ...(overridePositionUsdt !== undefined ? { overridePositionUsdt } : {}),
            });
            ...
            if (newShortPos) {
                ...
                if (cfg.paper.use_sqlite === true && trade) {
                    try {
                        const db = getDb(cfg.paper.scenarioId);
                        newShortPos.dbId = db.insertTrade(cfg.paper.scenarioId, signal.symbol, "short", newShortPos.quantity, trade.price, newShortPos.stopLoss, newShortPos.takeProfit, trade.timestamp);
                    }
                    catch (e) {
                        log.warn(`SQLite insert failed: ${e instanceof Error ? e.message : String(e)}`);
                    }
        ...
        trade = paperCoverShort(account, signal.symbol, signal.price, signal.reason.join(", "), paperOpts(cfg));
        if (trade && sigHistIdCover) {
            try {
                closeSignal(sigHistIdCover, signal.price, "signal", trade.pnl);
            }
            catch (e) {
                log.warn(`Signal history close failed: ${e instanceof Error ? e.message : String(e)}`);
            }
 ...
 */
function checkStagedTakeProfit(account, symbol, pos, currentPrice, cfg, triggered) {
    if (!pos.tpStages)
        return;
...
export function checkExitConditions(prices, cfg, strategy, ctx) {
    const sid = scenarioId(cfg);
    const account = loadAccount(cfg.paper.initial_usdt, sid);
    resetDailyLossIfNeeded(account);
    const triggered = [];
    ...
    for (const [symbol, pos] of Object.entries(account.positions)) {
        ...
        // ── P8.1 Break-Even Stop / Custom Stoploss ──
        {
            ...
            const newStop = resolveNewStopLoss(pos.side ?? "long", pos.entryPrice, pos.stopLoss, currentPrice, profitRatio, holdMs, symbol, cfg.risk, strategy, ctx);
            if (newStop !== null) {
        ...
        if (hitStopLoss) {
            ...
            const skipTrailing = onlyOffset === true &&
            ...
            if (!skipTrailing) {
                ...
                const callbackPct = pos.trailingStopActivated && positivePct !== undefined
                    ? positivePct * 100
        ...
        if (exitReason) {
            // ── P8.2 Exit confirmation hook ──────────────────────────────────────
            {
                ...
                const confirmResult = shouldConfirmExit({ symbol, side: pos.side ?? "long", entryPrice: pos.entryPrice, currentPrice, profitRatio, holdMs }, exitReason, maxDev, strategy, ctx);
                if (!confirmResult.confirmed) {
                    const cooling = isExitRejectionCoolingDown(symbol, cooldownSec * 1000, _exitRejectionLog);
                    if (!cooling) {
                        log.info(`[confirm-exit] ${symbol} exit rejected (reason: ${confirmResult.reason ?? "unknown"}, exitReason: ${exitReason})`);
            ...
            if (trade) {
                ...
                if (sigHistId) {
                    try {
                        closeSignal(sigHistId, currentPrice, exitReason, trade.pnl);
                    }
                    catch (e) {
                        log.warn(`Signal history close failed: ${e instanceof Error ? e.message : String(e)}`);
                    }
                ...
                if (cfg.paper.use_sqlite === true && posDbId !== undefined) {
                    try {
                        const db = getDb(cfg.paper.scenarioId);
                        db.closeTrade(posDbId, currentPrice, trade.pnl ?? 0, pnlPercent / 100, exitReason === "stop_loss" || exitReason === "trailing_stop", exitReason === "take_profit", Date.now());
                    }
                    catch (e) {
                        log.warn(`SQLite close failed: ${e instanceof Error ? e.message : String(e)}`);
                    }
                }
            }
            continue;
...
export function checkStopLoss(prices, cfg) {
...
export function checkMaxDrawdown(prices, cfg) {
...
export function checkDailyLossLimit(prices, cfg) {
    const account = loadAccount(cfg.paper.initial_usdt, scenarioId(cfg));
    resetDailyLossIfNeeded(account);
    const equity = calcTotalEquity(account, prices);
 ...
 */
export function checkDcaTranches(prices, cfg, strategy, ctx) {
    const sid = scenarioId(cfg);
    const account = loadAccount(cfg.paper.initial_usdt, sid);
    const dcaCfg = cfg.risk.dca;
    if (!dcaCfg?.enabled)
            ...
            if (adjustAmount !== null && adjustAmount !== 0) {
                if (adjustAmount > 0) {
                    ...
                    if (account.usdt >= adjustAmount) {
                        const trade = paperDcaAdd(account, symbol, currentPrice, `adjustPosition add $${adjustAmount.toFixed(2)}`, { addUsdt: adjustAmount, feeRate: 0.001 });
                        if (trade) {
                            executed.push({
                                symbol,
                                trade,
                    ...
                    if (reduceQty > 0 && reduceQty <= pos.quantity) {
                        const trade = paperSell(account, symbol, currentPrice, `adjustPosition reduce $${reduceUsdt.toFixed(2)}`, { ...paperOpts(cfg), overrideQty: reduceQty });
                        if (trade) {
                            executed.push({
                                symbol,
                                trade,
...
export function getPaperSummary(prices, cfg) {
    return getAccountSummary(loadAccount(cfg.paper.initial_usdt, scenarioId(cfg)), prices);
}
export function formatSummaryMessage(prices, cfg, mode = "full") {
    ...
    if (summary.positions.length > 0) {
        ...
        for (const pos of summary.positions) {
            ...
            lines.push(`  ${posEmoji} ${dirLabel} ${pos.symbol}: $${pos.entryPrice.toFixed(4)} → $${pos.currentPrice.toFixed(4)} | ${posSign}${(pos.unrealizedPnlPercent * 100).toFixed(2)}%`, `     SL: $${pos.stopLoss.toFixed(4)} | TP: $${pos.takeProfit.toFixed(4)}`);
        }
    }
    else {
    ...
    if (mode === "full") {
        lines.push(``, `📈 Total Trades: ${summary.tradeCount}`, `🎯 Win Rate: ${summary.tradeCount > 0 ? (summary.winRate * 100).toFixed(0) + "%" : "N/A"}`);
    }
    return lines.join("\n");
}
