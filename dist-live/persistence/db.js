...
export class TradeDB {
     ...
     */
    insertTrade(scenarioId, symbol, side, quantity, entryPrice, stopLoss, takeProfit, openAt) {
     ...
     */
    closeTrade(id, exitPrice, pnl, pnlRatio, wasStopLoss, wasRoiExit, closedAt) {
        const stmt = this.db.prepare(`
      UPDATE trades
     ...
     */
    getRecentClosedTrades(scenarioId, sinceMs) {
        const stmt = this.db.prepare(`
    ...
    `);
        const rows = stmt.all(scenarioId, sinceMs);
        return rows.map((r) => ({
            symbol: r.symbol,
            closedAt: r.closedAt,
            pnlRatio: r.pnlRatio,
     ...
     */
    recordSnapshot(scenarioId, equity, cash, openPositions, snapshotAt) {
        const stmt = this.db.prepare(`
