...
export function getEquityHistoryPath(scenarioId) {
 ...
 */
export function recordEquitySnapshot(scenarioId, equity, positions) {
    ...
    const snapshot = { timestamp: now, equity, positions };
 ...
 */
export function loadEquityHistory(scenarioId, sinceDaysAgo) {
    const filePath = getEquityHistoryPath(scenarioId);
    if (!fs.existsSync(filePath))
        return [];
    const cutoff = sinceDaysAgo !== undefined ? Date.now() - sinceDaysAgo * 24 * 60 * 60 * 1000 : 0;
    const content = fs.readFileSync(filePath, "utf-8");
    const results = [];
