...
const defaultStrategy = {
    ...
    description: "Uses signals.buy/sell/short/cover conditions from strategy.yaml / strategies/*.yaml for signal matching. Preserves original behavior without any modifications.",
    populateSignal(ctx) {
        // Directly reuse existing detectSignal, maintaining full consistency (including position-aware logic)
        const signal = detectSignal("plugin_call", ctx.indicators, ctx.cfg, ctx.currentPosSide);
        return signal.type;
    },
};
