...
const ensembleStrategy = {
    ...
    description: "Merges signals from multiple strategies via weighted voting. Supports threshold (minimum confidence) and unanimous (all-agree) modes.",
    populateSignal(ctx) {
        // Read ensemble config from configuration
        const ensembleCfg = ctx.cfg.ensemble;
        if (!ensembleCfg) {
