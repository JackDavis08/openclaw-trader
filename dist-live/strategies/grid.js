 ...
 */
export function computeGridLevels(upper, lower, count, type) {
    if (count < 2 || upper <= lower || lower <= 0)
        return [];
    const levels = [];
 ...
 */
export function autoDetectRange(klines, lookback) {
...
const gridStrategy = {
    id: "grid",
    name: "Grid Trading",
    description: "Range-bound grid trading: buys at unfilled grid levels, DCA adds at lower levels, " +
    ...
    populateSignal(ctx) {
        ...
        if (!gridCfg?.enabled || !stateStore)
            return "none";
        // Initialize or load grid state
        let state = stateStore.get(GRID_STATE_KEY, null);
        if (!state) {
    ...
    adjustPosition(position, ctx) {
        const { cfg, stateStore } = ctx;
        const gridCfg = cfg.strategy.grid;
        if (!gridCfg?.enabled || !stateStore)
            return null;
        const state = stateStore.get(GRID_STATE_KEY, null);
        if (!state)
            return null;
    ...
    shouldExit(position, ctx) {
        const { cfg, stateStore } = ctx;
        const gridCfg = cfg.strategy.grid;
        if (!gridCfg?.enabled || !stateStore)
            return null;
        const state = stateStore.get(GRID_STATE_KEY, null);
        if (!state || state.filledLevels.length === 0)
            return null;
        const price = position.currentPrice;
