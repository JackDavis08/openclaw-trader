 ...
 */
export function calcBreakEvenStop(side, entryPrice, currentStopLoss, profitRatio, breakEvenProfit, breakEvenStop) {
    // Trigger condition not met
    if (profitRatio < breakEvenProfit)
        return null;
 ...
 */
export function resolveNewStopLoss(side, entryPrice, currentStopLoss, currentPrice, profitRatio, holdMs, symbol, riskCfg, strategy, ctx) {
    ...
    if (candidateStop === null) {
        ...
        if (bep !== undefined) {
            candidateStop = calcBreakEvenStop(side, entryPrice, currentStopLoss, profitRatio, bep, bes);
        }
    }
    ...
    if (side === "long") {
        if (candidateStop <= currentStopLoss)
            return null;
    }
    else {
        if (candidateStop >= currentStopLoss)
