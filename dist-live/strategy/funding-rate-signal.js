...
export function readFundingRateCache(symbol, maxAgeMs = 10 * 60_000) {
    try {
        if (!fs.existsSync(FR_CACHE_PATH))
            return undefined;
        const cache = JSON.parse(fs.readFileSync(FR_CACHE_PATH, "utf-8"));
...
export function writeFundingRateCache(symbol, ratePct) {
    let cache = {};
    try {
        if (fs.existsSync(FR_CACHE_PATH)) {
            cache = JSON.parse(fs.readFileSync(FR_CACHE_PATH, "utf-8"));
        }
    }
 ...
 */
export function checkFundingRateSignal(ratePct, longThreshold = 0.30, shortThreshold = 0.15) {
    if (ratePct > longThreshold)
        return "overlong";
    if (ratePct < -shortThreshold)
        return "overshort";
    return "neutral";
}
 ...
 */
export async function fetchFundingRatePct(symbol) {
    // Read cache first
