...
export function macd(closes, fastPeriod = 12, slowPeriod = 26, signalPeriod = 9) {
    ...
    const prevPrevHistogram = signalArr.length >= 3 ? prevPrevMacd - prevPrevSignalLine : undefined;
    return {
        macd: currentMacd,
        signal: signalLine,
        histogram,
 ...
 */
export function calcAtrPositionSize(totalUsdt, price, atrValue, riskPercent = 0.02, atrMultiplier = 1.5, maxRatio = 0.3) {
    if (isNaN(atrValue) || atrValue <= 0 || price <= 0)
        return totalUsdt * 0.1; // fallback
 ...
 */
export function calcVwap(klines) {
    if (klines.length === 0)
        return null;
...
export function calculateIndicators(klines, maShortPeriod, maLongPeriod, rsiPeriod, macdConfig) {
    if (klines.length < maLongPeriod + 1)
        return null;
    ...
    if (isNaN(maShort) || isNaN(maLong) || isNaN(rsiValue))
        return null;
