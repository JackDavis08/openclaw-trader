 ...
 */
export function calcBollingerWidth(closes, period = 20, stdDevMult = 2) {
 ...
 */
export function analyzePriceStructure(klines, lookback = 10) {
    ...
    if (higherHigh && higherLow)
        return "higher_highs"; // Strong upward structure
    if (lowerLow && lowerHigh)
        return "lower_lows"; // Strong downward structure
    if (higherHigh || lowerLow)
        return "mixed"; // Mixed (wide ranging or turning point)
 ...
 */
export function classifyRegime(klines) {
...
function calcBollingerWidthHistory(closes, period, lookback) {
    const result = [];
 ...
 */
export function shouldAllowSignal(regime, signalType) {
    ...
    switch (signalFilter) {
        case "trend_signals_only":
            ...
            if (regime.regime === "trending_bull")
                return signalType === "buy" || signalType === "cover";
            if (regime.regime === "trending_bear")
                return signalType === "short" || signalType === "sell";
            return true;
 ...
 */
export function formatRegimeReport(symbol, regime) {
