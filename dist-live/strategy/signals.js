...
const SIGNAL_CHECKERS = {
    // ── MA Trend ──────────────────────────────────────
    /** MA golden cross: short-term crosses above long-term */
    ma_golden_cross: (ind) => ind.prevMaShort !== undefined &&
    ...
    ma_death_cross: (ind) => ind.prevMaShort !== undefined &&
        ind.prevMaLong !== undefined &&
        ind.prevMaShort >= ind.prevMaLong &&
        ind.maShort < ind.maLong,
    ...
    macd_golden_cross: (ind) => !!ind.macd &&
    ...
    macd_histogram_expanding: (ind) => !!ind.macd &&
    ...
    macd_histogram_shrinking: (ind) => {
        ...
        if (prevPrevHistogram === undefined)
            return twoBarShrink; // Fallback to two-bar detection
     ...
     */
    price_above_vwap: (ind) => ind.vwap !== undefined && ind.price > ind.vwap,
    /**
     * Price below VWAP = bears dominate
     * Below institutional average daily cost, higher short win rate
     */
    price_below_vwap: (ind) => ind.vwap !== undefined && ind.price < ind.vwap,
    /**
     ...
     */
    vwap_bounce: (ind) => ind.vwap !== undefined &&
     ...
     */
    vwap_breakdown: (ind) => ind.vwap !== undefined &&
        ind.prevPrice !== undefined &&
        ind.prevPrice >= ind.vwap &&
        ind.price < ind.vwap,
    /**
     ...
     */
    price_above_vwap_upper2: (ind) => ind.vwapUpper2 !== undefined && ind.price > ind.vwapUpper2,
    /**
     * Price oversold (below VWAP - 2 sigma)
     * Statistically oversold, can serve as bottom-fishing or reversal entry reference
     */
    price_below_vwap_lower2: (ind) => ind.vwapLower2 !== undefined && ind.price < ind.vwapLower2,
    // ── BTC Dominance Trend ────────────────────────────────
    /**
     ...
     */
    btc_dominance_rising: (ind) => ind.btcDomChange !== undefined && ind.btcDomChange > 0.5,
    /**
     ...
     */
    btc_dominance_falling: (ind) => ind.btcDomChange !== undefined && ind.btcDomChange < -0.5,
    ...
    ls_ratio_extreme_long: (ind, cfg) => {
        if (ind.longShortRatio === undefined)
            return false;
    ...
    pcr_extreme_bearish: (ind, cfg) => {
        if (ind.putCallRatio === undefined)
            return false;
        const threshold = cfg.strategy
     ...
     */
    stablecoin_not_accumulation: (ind) => ind.stablecoinSignal === undefined || ind.stablecoinSignal !== "accumulation",
};
/** Internal helper: check if all conditions in a group are met, returns [allMet, metConditionNames] */
function checkConditions(conditions, indicators, cfg, label) {
 ...
 */
export function detectSignal(symbol, indicators, cfg, positionSide) {
    ...
    if (positionSide === "long") {
        ...
        if (sellMet)
            return makeSignal("sell", sellReasons);
    ...
    if (positionSide === "short") {
        ...
        if (coverMet)
            return makeSignal("cover", coverReasons);
        return makeSignal("none", []);
    }
