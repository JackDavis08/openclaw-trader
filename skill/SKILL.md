...
## Common Issues & Recovery (FAQ)

### Kill switch triggered unexpectedly
```bash
npm run paper:reset -- --kill-switch    # deactivate
npm run doctor                           # verify state
```
Root causes: initialUsdt mismatch, position reconciliation wipe on restart.

### initialUsdt mismatch (state file vs paper.yaml)
The `paper-*.json` state files persist `initialUsdt` independently. Fix:
```bash
npm run paper:reset -- --scenario <id> --set-initial <value>
```
Doctor will warn if these diverge on startup.

### Futures Testnet -2015 error
Using Spot Testnet key on Futures endpoint — they're separate systems.
- Spot Testnet: https://testnet.binance.vision (GitHub login)
- Futures Testnet: https://testnet.binancefuture.com (separate registration)
Credentials stored in `.secrets/binance-testnet.json` and `.secrets/binance-futures-testnet.json` respectively.

### MAX_NUM_ALGO_ORDERS (spot testnet)
Spot testnet limits concurrent stop-loss/take-profit orders to 5.
System automatically falls back to local price-polling when this is hit — no action needed.

### STOP_MARKET rejected on futures testnet
Futures testnet rejects conditional orders via standard REST API.
System automatically falls back to local price-polling — no action needed.

### Monitor not producing signals
Check in order:
1. `npm run doctor` — look for log staleness warnings
2. Confirm `config/paper.yaml` has the scenario with `enabled: true`
3. Check R:R filter — most signals are blocked by R:R < min_rr (default 1.5)
4. Check kill switch: `logs/kill-switch-state.json`
5. Check news emergency halt: `logs/news-emergency.json`

### New user first-time setup
```bash
npm run setup    # interactive wizard: credentials + paper config + cron sync
npm run doctor   # verify everything is working
npm run live     # start monitoring
```
